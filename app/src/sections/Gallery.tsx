        ...
        <div className="flex w-full overflow-hidden py-2 mask-gradient">
          <div className="flex w-max animate-marquee-ltr hover:[animation-play-state:paused] cursor-pointer">
            {row1.map((item, i) => (
              <div
                key={i}
                className="mx-4 flex flex-col justify-center bg-[#f8fafc] border border-[#e2e8f0] px-8 py-5 rounded-2xl shadow-sm hover:border-[#1d6fc4] hover:shadow-[0_8px_30px_rgba(29,111,196,0.08)] hover:-translate-y-1 transition-all duration-300 min-w-[220px]"
              >
                <span className="font-['Poppins'] font-bold text-[18px] text-[#0f2b5c]">
                   {item.name}
                </span>
                <span className="font-mono text-[12px] text-[#1d6fc4] mt-1.5 opacity-80">
                  {item.formula}
                </span>
              </div>
            ))}
